 ...
 * 
 */
@XmlRootElement(name = "TestHeartbeatWithNotificationAndSync")
@XmlAccessorType(XmlAccessType.FIELD)
@XmlAddToContext
public class TestHeartbeatWithNotificationAndSync extends DefaultCommandListener {
    ...
    public TestHeartbeatWithNotificationAndSync() {
        secondTime = false;
    }

    public TestHeartbeatWithNotificationAndSync(final String nodeName) {
        this.nodeName = nodeName;
        this.filename = "/"+nodeName+".xml";
    }

    private static Logger logger = LoggerFactory.getLogger(TestHeartbeatWithNotificationAndSync.class);
    ...
    public void get(final String messageId, final Filter filter, final PrintWriter out) {
	  if (filter != null && filter.asString() != null && filter.asString().contains("streams")) {
	    ...
	    out.println("\t\t\t\t\t<replayLogAgedTime>2015-07-01T00:00:00Z</replayLogAgedTime>");
	    out.println("\t\t\t\t</stream>");
	    out.println("\t\t\t</streams>");
	    out.println("\t\t</netconf>");
	    out.println("\t</data>");
...
*/
	...
	case MERGE: // used for HEARTBEAT that is missing in the enum
             notificationType = "<eventTime>2015-02-12T08:45:01Z</eventTime>"
